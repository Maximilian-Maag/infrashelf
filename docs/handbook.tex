...
\paragraph{Passwordless sign-in.} A key can also be the \emph{first} factor: the
sign-in page offers \emph{Use security key} above the email field, and the
ceremony names no account at all. The options carry no \code{allowCredentials},
so the browser offers whatever discoverable credential it holds for this RP id,
and the account is resolved from the credential that answers --- cross-checked
against the user handle the authenticator returns, which is the database id as
registration wrote it.

That ceremony requires \emph{user verification}, and that is what makes it two
factors rather than one: with no password in front of it, the PIN or biometric
releasing the key is the second factor. An administrator signing in this way is
therefore not asked for a code on top --- the mandatory-second-factor rule is
satisfied by the ceremony itself.

The challenge lives in \code{webauthn\_login\_challenges}, keyed on the challenge
rather than on a user, because there is no user to key it on when it is issued.
Single use comes from the same \code{DELETE ... RETURNING} claim the keyed table
uses.

Only a \emph{discoverable} credential can answer. Registration asks for
\code{residentKey: 'preferred'} and records what the authenticator actually made
via the \code{credProps} extension, so a passkey becomes usable for passwordless
sign-in automatically while an older hardware key remains a perfectly good second
factor. A credential registered before this existed was never asked, so it starts
as unknown and earns the flag the first time it answers such a ceremony.
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}lp{9cm}@{}}
    ...
    \code{webauthn\_login\_challenges} & In-flight passwordless challenges, keyed on the challenge because no user is known yet. Single-use and short-lived \\
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}llll@{}}
    ...
    \code{last\_refresh\_outcome} & TEXT & NULL & what the last drift report found (\code{clean}, \code{drifted}, \code{locked}, \code{error}); NULL is \emph{never checked}, not \emph{checked and clean} \\
    \code{drift\_detected\_at}    & TIMESTAMPTZ & NULL & when drift was found; cleared by a later clean report \\
    \code{drift\_summary}         & JSONB & NULL & the resources the report saw move \\
    \code{policy\_checked\_at}    & TIMESTAMPTZ & NULL & when policy was last asked about this element (NULL: never asked) \\
    \code{policy\_outcome}        & TEXT & NULL & \code{allow}/\code{warn}/\code{deny}/\code{needs-approval} from the engine, or \code{unavailable} when it could not be asked --- reported on the element, never enforced \\
    \code{policy\_rule}           & TEXT & NULL & which rule decided \\
    \code{policy\_message}        & TEXT & NULL & the policy's own words, shown as written \\
    \bottomrule
  \end{tabular}
\end{table}
...
Under concurrency the block holds. The check and the insert are one
transaction, and it locks the cost centre before reading the committed
figure, so two orders placed against one cost centre in the same instant
cannot both pass a check only one should have --- the second waits for
the first to commit and is measured against it. The cost is that
ordering against a \emph{single} cost centre is serialised at checkout;
different cost centres do not wait for each other, and provisioning,
email and audit entries all happen after the lock is released.
