...
| `SECRET_ENCRYPTION_KEY` | **Effectively yes** | 64 hex characters (`openssl rand -hex 32`) encrypting every stored credential: external-system integrations (Foreman, Ansible, Nexus, Pulp, Loki, Grafana) **and CI source access tokens** (#111). Blank means no new credential of either kind can be stored — those endpoints answer 503 rather than writing plain text — and CI source tokens already in the database stay plain text until a key is set, at which point the backend encrypts them once at boot. Not rotatable in place: a new key cannot decrypt what the old one wrote |
