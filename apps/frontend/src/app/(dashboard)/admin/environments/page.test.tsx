...
vi.mock('./EnvironmentsManager', () => ({
  EnvironmentsManager: ({ initial, initialError }: { initial: unknown[]; initialError: string | null }) =>
...
describe('EnvironmentsPage', () => {
  it('sends a caller with no session to the login page', async () => {
    ...
    await expect(EnvironmentsPage()).rejects.toThrow('NEXT_REDIRECT')
  ...
  it('refuses anyone who is not root', async () => {
    ...
    for (const role of ['admin', 'project_manager']) {
      ...
      await expect(EnvironmentsPage(), role).rejects.toThrow('NEXT_REDIRECT')
      expect(redirect).toHaveBeenCalledWith('/admin')
    }
  })

  it('fetches the rows and hands them to the manager', async () => {
    render(await EnvironmentsPage())
  ...
  it('hands over the REASON when the fetch failed, not an empty list', async () => {
    ...
    expect(console.error).toHaveBeenCalledWith('[page] could not load deployment environments: HTTP 502: Bad Gateway')
  })
})
