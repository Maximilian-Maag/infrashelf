...
const focusProbe = () => {
  ...
  const shadowColours = (shadow: string): { colour: [number, number, number, number]; inset: boolean }[] => {
    ...
    return shadow
      ...
      // seen. Tailwind authors `oklch()`, but what reaches `getComputedStyle`
      // is whatever the build's CSS optimiser emitted — Next 16's turns all 58
      // of them into `lab()`, and a name-by-name allow-list reported every ring
      // in the app as absent. The file already learned this once with `oklch`.
      // Longest names first, and a lookbehind, so `oklab(` is not split as `lab(`.
